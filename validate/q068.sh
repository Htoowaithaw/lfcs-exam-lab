...
marker="LFCS-2-VALIDATE-$$"
logger -t lfcs-op-log2-tag "$marker"
for _ in {1..20}; do grep -Fq "$marker" /var/log/lfcs-op-log2.log 2>/dev/null && break; sleep 0.25; done
grep -Fq "$marker" /var/log/lfcs-op-log2.log || { echo "RESULT: FAIL - live rsyslog routing is not working"; exit 1; }
echo "RESULT: PASS"

...
grep -Eq '(^|[[:space:]])PID([[:space:]]|$)' /root/lfcs-op-mon12.txt || { echo "RESULT: FAIL - ps PID header missing"; exit 1; }
awk '$1 ~ /^[0-9]+$/ {found=1} END {exit !found}' /root/lfcs-op-mon12.txt || { echo "RESULT: FAIL - process snapshot has no process row"; exit 1; }
awk '$1 ~ /^[0-9]+$/ && NF >= 10 {found=1} END {exit !found}' /root/lfcs-op-mon12.txt || { echo "RESULT: FAIL - vmstat snapshot has no data row"; exit 1; }
echo "RESULT: PASS"

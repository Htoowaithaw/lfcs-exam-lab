...
aa-status 2>/dev/null | awk '/profiles are in enforce mode/ {inside=1; next} /profiles are in complain mode/ {inside=0} inside && $1=="/usr/local/bin/lfcs-op-aa2" {found=1} END {exit !found}' || { echo "RESULT: FAIL - AppArmor profile is not in enforce mode"; exit 1; }
echo "RESULT: PASS"

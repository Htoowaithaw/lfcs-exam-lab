...
virsh domstate lfcs-op-vm3 | grep -Eq '^(shut off|shutoff)$' || { echo "RESULT: FAIL - domain must remain powered off"; exit 1; }
virsh pool-dumpxml lfcs-op-vm3-pool | grep -Fq '<path>/var/lib/libvirt/lfcs-op-vm3-pool</path>' || { echo "RESULT: FAIL - storage pool target path is wrong"; exit 1; }
echo "RESULT: PASS"

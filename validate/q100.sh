...
openssl pkey -in /etc/ssl/private/lfcs-op-tls1.key -text -noout 2>/dev/null | grep -q 'Private-Key: (2048 bit' || { echo "RESULT: FAIL - RSA key is not 2048 bits"; exit 1; }
[ "$(openssl x509 -in /etc/ssl/certs/lfcs-op-tls1.crt -noout -subject | sed 's/^subject=//')" = "$(openssl x509 -in /etc/ssl/certs/lfcs-op-tls1.crt -noout -issuer | sed 's/^issuer=//')" ] || { echo "RESULT: FAIL - certificate is not self-signed"; exit 1; }
echo "RESULT: PASS"

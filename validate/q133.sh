#!/usr/bin/env bash
set -euo pipefail
if ! getent passwd sshuser2 >/dev/null; then echo "RESULT: FAIL - ssh test user missing"; exit 1; fi
if ! test -s /home/sshuser2/.ssh/authorized_keys; then echo "RESULT: FAIL - authorized_keys missing"; exit 1; fi

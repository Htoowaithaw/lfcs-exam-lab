...
findmnt -no OPTIONS /mnt/lfcs-opt3 | tr ',' '\n' | grep -qx 'nodev' || fail 'mount option is not active'
awk '$2 == "/mnt/lfcs-opt3" && $3 == "ext4" && $4 ~ /(^|,)nodev(,|$)/ { found=1 } END { exit !found }' /etc/fstab || fail 'mount option is not persistent for requested mountpoint'
mount -a || fail 'mount -a failed'
spec="$(awk '$2=="/mnt/lfcs-opt3" && $3=="ext4" && $4 ~ /(^|,)nodev(,|$)/ {print $1; exit}' /etc/fstab)"

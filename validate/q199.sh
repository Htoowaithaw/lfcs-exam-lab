...
findmnt -rn -o OPTIONS /mnt/lfcs-nfs1 2>/dev/null | tr ',' '
...
if touch /mnt/lfcs-nfs1/.lfcs-write-test >/dev/null 2>&1; then rm -f /mnt/lfcs-nfs1/.lfcs-write-test; fail 'NFS export is writable from node1'; fi
awk '$1=="192.168.56.12:/srv/lfcs-nfs1" && $2=="/mnt/lfcs-nfs1" && $3=="nfs" && $4 ~ /(^|,)ro(,|$)/ && $4 ~ /(^|,)_netdev(,|$)/ {found=1} END {exit !found}' /etc/fstab || fail 'persistent read-only NFS fstab entry missing'

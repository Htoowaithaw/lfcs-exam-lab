...
size=$(blockdev --getsize64 /dev/nbd1 2>/dev/null || echo 0)
...
if ! timeout 10 nbd-client 192.168.56.12 -N lfcsnbd2 /dev/nbd7 >/dev/null 2>&1; then fail 'node2 NBD export name is missing'; fi
nbd-client -d /dev/nbd7 >/dev/null 2>&1 || true
echo "RESULT: PASS"

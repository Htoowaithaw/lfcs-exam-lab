...
grep -Eq '^[[:space:]]+User[[:space:]]+remoteuser1$' /root/.ssh/config || fail 'ssh User is wrong'
grep -Eq '^[[:space:]]+IdentityFile[[:space:]]+/root/.ssh/lfcs-q225$' /root/.ssh/config || fail 'ssh IdentityFile is wrong'
...
ssh -F /root/.ssh/config -o BatchMode=yes lfcs-node2-1 'test "$(id -un)" = "remoteuser1"' || fail 'non-interactive SSH to node2 failed'
echo "RESULT: PASS"
